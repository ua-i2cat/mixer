...
void*  init_encoder(video_frame **frame);
void*  init_layout_decoder(int width, int height);
...
int main(int argc, char *argv[]) {
	...
	struct state_decompress *sd1, *sd2, *sd3, *sd4, *sd5, *sd6, *sd7, *sd8, *sd_layout;
	struct compress_state *sc_layout = NULL;
...
//	b8=(uint8_t *)av_malloc(avpicture_get_size(PIX_FMT_UYVY422, pCodecCtx8.width, pCodecCtx8.height)*sizeof(uint8_t));
	...
	struct video_frame *frame = NULL, *out_frame = NULL;

	sc_layout = (struct compress_state *)init_encoder(&frame);
	sd_layout = (struct state_decompress *)init_layout_decoder(lay_width,lay_height);
	out_frame = vf_alloc (1);

	frame->tiles->width = lay_width;
	frame->tiles->height = lay_height;

	uint8_t* out_enc_buff = (uint8_t*)av_malloc(avpicture_get_size(PIX_FMT_RGB24, lay_width, lay_height)*sizeof(uint8_t));

	//Init layout
	Layout layout;
	layout.init(lay_width, lay_height, PIX_FMT_RGB24, 8);
...
//    	layout.introduce_frame(id8, b8);
    	...
    	AVFrame *out_AVframe = avcodec_alloc_frame();
    	uint8_t** lay_data = layout.get_layout_bytestream();
    	frame->tiles->data = (char*)lay_data[0];
    	frame->tiles->data_len = layout.get_h() * layout.get_lay_frame()->linesize[0];
    	frame->tiles->linesize = layout.get_lay_frame()->linesize[0];

    	out_frame = compress_frame(sc_layout, frame , 0);

    	if(decompress_frame(sd_layout, out_enc_buff, (uint8_t*)out_frame->tiles->data, out_frame->tiles->data_len, frame_cont)==1){
    		avpicture_fill((AVPicture *)out_AVframe, out_enc_buff, PIX_FMT_RGB24, layout.get_w(), layout.get_h());
...
void*  init_layout_decoder(int width, int height){
	...
	decompress_reconfigure(sd, des, 0, 0, 0, vc_get_linesize(width, RGB), RGB);
...
void SaveFrame(AVFrame *pFrame, int width, int height, int iFrame) {
  ...
  // Open file
  sprintf(szFilename, "/home/palau/TFG/layout_prints/decoded_encoded1/layout%d.ppm", iFrame);
...
void*  init_encoder(video_frame **frame){
	...
	cs = (struct compress_state *) calloc(2, sizeof(struct compress_state *));

	printf("Give me some compressor help\n");
	show_compress_help();

	printf("Trying to initialize encoder\n");
	compress_init("libavcodec:codec=H.264", &cs);
	printf("Encoder initialized ;^)\n");

	struct video_desc des_enc;

	des_enc.width = 1920;
	des_enc.height = 1080;
	des_enc.color_spec  = RGB;
	des_enc.tile_count = 1;
	des_enc.interlacing = PROGRESSIVE;
	des_enc.fps = 25;

	*frame = vf_alloc_desc_data(des_enc);
